...
class vowelsforminc
{
    ...
    /**
     * Show a Vowelsform referral link under the form?
     * @var boolean
     */
     ...
     * @var string
     */
    protected $_referralText = 'Powered by Vowelsform';
    ...
    {
        $description = apply_filters('vowels_form_builder_description', $this->_description, $this);
        $description = apply_filters('vowels_form_builder_description_' . $this->getId(), $description, $this);
        return $description;
    }
    ...
    {
        $successMessage = apply_filters('vowels_form_builder_success_message', $this->_successMessage, $this);
        $successMessage = apply_filters('vowels_form_builder_success_message_' . $this->getId(), $successMessage, $this);
    ...
    public function getSuccessRedirectURL()
    {
        ...
        $url = apply_filters('vowels_form_builder_success_redirect_url', $url, $this);
        $url = apply_filters('vowels_form_builder_success_redirect_url_' . $this->getId(), $url, $this);
    ...
    {
        return (strlen($this->_submitButtonText)) ? $this->_submitButtonText : _x('Send', 'submit the form','vowels-contact-form-with-drag-and-drop');
    }
    ...
    {
        if (strlen($colour) > 0) {
            $this->_elementBackgroundColour = vowelsforminc::parseCss('background-color: ' . $colour . ';');
        } else {
    ...
    {
        if (strlen($colour) > 0) {
            $this->_elementBorderColour = vowelsforminc::parseCss('border: 1px solid ' . $colour . ';');
        } else {
    ...
    {
        if (strlen($colour) > 0) {
            $this->_elementTextColour = vowelsforminc::parseCss('color: ' . $colour . ';');
        } else {
    ...
    {
        if (strlen($colour) > 0) {
            $this->_labelTextColour = vowelsforminc::parseCss('color: ' . $colour . ';');
        } else {
     ...
     *
     * @return vowelsforminc_Element_Email
     */
    ...
    public function getNotificationFromInfo()
    {
        ...
        if ($this->getNotificationFromType() == 'element') {
            ...
            if ($notificationFromElement instanceof vowelsforminc_Element_Email && strlen($notificationFromElement->getValue())) {
    ...
    public function getAutoreplyFromInfo()
    {
        ...
        if ($this->getAutoreplyFromType() == 'element') {
            ...
            if ($autoreplyFromElement instanceof vowelsforminc_Element_Email && strlen($autoreplyFromElement->getValue())) {
    ...
    /**
     * Get the JavaScript configuration for the vowelsforminc plugin
     *
     * @return string
     */
    public function getJsConfig()
    {
        ...
        if (strpos(vowels_form_builder_get_http_referer(), 'vowels_form_builder_preview=1')) {
            $config['preview'] = true;
        }

        return vowels_form_builder_json_encode($config);
    }
     ...
     *
     * @param $element vowelsforminc_Element The element to add
     */
    public function addElement(vowelsforminc_Element $element)
    {
        $this->_elements[$element->getName()] = $element;

        if ($element instanceof vowelsforminc_Element_Date && $element->getShowDatepicker()) {
     ...
     *
     * @param string $name
     * @return null|vowelsforminc_Element
     */
    ...
    public function setValue($name, $value)
    {
        ...
        if ($element instanceof vowelsforminc_Element) {
    ...
    public function getValue($name)
    {
        ...
        return $element instanceof vowelsforminc_Element ? $element->getValue() : null;
    }
    ...
    public function getValueHtml($name)
    {
        ...
        return $element instanceof vowelsforminc_Element ? $element->getValueHtml() : '';
    }
    ...
    public function getValuePlain($name)
    {
        ...
        return $element instanceof vowelsforminc_Element ? $element->getValuePlain() : '';
    }
    ...
    public static function autoload($class)
    {
        ...
        if (strpos($class, 'vowelsforminc') !== 0) {
            return false;
        }
    ...
    /**
     * Configure autoloading of vowelsforminc classes
     */
    public static function registerAutoload()
    {
        spl_autoload_register(array('vowelsforminc', 'autoload'));
    ...
    public function fromConfig(array $config)
    {
        ...
        if (array_key_exists('uniq_id', $config)) {
            ...
            $uniqueId = uniqid();
            $uniqueId = apply_filters('vowels_form_builder_form_unique_id', $uniqueId , $this);
            $uniqueId = apply_filters('vowels_form_builder_form_unique_id_' . $this->getId(), $uniqueId, $this);
        ...
        if (array_key_exists('elements', $config) && is_array($config['elements'])) {
            foreach ($config['elements'] as $eConfig) {
                if (isset($eConfig['type'])) {
                    $eClass = 'vowelsforminc_Element_' . ucfirst($eConfig['type']);
                    if (class_exists($eClass)) {
                        $eConfig['name'] = 'vowels_form_builder_' . $this->getId() . '_' . $eConfig['id'];
        ...
        if (array_key_exists('use_honeypot', $config) && $config['use_honeypot']) {
            $honeypot = new vowelsforminc_Element_Honeypot(array(
                'name' => 'vowels_form_builder_' . $this->getId() . '_0',
                'unique_id' => 'vowels_form_builder_' . $this->getUniqId() . '_0',
                'form' => $this
            ));
    ...
    public function getConditionalLogicJson()
    {
        ...
        return vowels_form_builder_json_encode(array('logic' => $logic, 'dependents' => $dependents, 'animate' => $this->getConditionalLogicAnimation()));
    }
    ...
    {
        return preg_replace_callback('/{.+}/U', array('vowelsforminc', 'getPlaceholderValue2'), $string);
    }
    ...
    public static function getPlaceholderValue2($matches)
    {
        ...
        switch ($original) {
            case '{ip}':
                return vowels_form_builder_get_user_ip();
            case '{user_agent}':
                return vowels_form_builder_get_user_agent();
            case '{post_id}':
                return vowels_form_builder_get_current_post_id();
            case '{post_title}':
                return vowels_form_builder_get_current_post_title();
            case '{url}':
                return vowels_form_builder_get_current_url();
     ...
     protected function _getPlaceholderValue($matches)
    {
        ...
        switch ($original) {
            case '{post_id}':
				if( isset( $_POST['post_id'] ) ){$valid['post_id'] = sanitize_text_field($_POST['post_id'] ) ? sanitize_text_field($_POST['post_id']) : '';}
                return $valid['post_id'];
				
			//	isset($_POST['post_id']) ? $_POST['post_id'] : '';
				
	
            case '{post_title}':
				if( isset( $_POST['post_title'] ) ){$valid['post_title'] = sanitize_text_field($_POST['post_title'] ) ? sanitize_text_field($_POST['post_title']) : '';}
                return $valid['post_title'];
			//	isset($_POST['post_title']) ? $_POST['post_title'] : '';
				
            case '{referring_url}':
			    if( isset( $_POST['referring_url'] ) ){$valid['referring_url'] = esc_url($_POST['referring_url'] ) ? esc_url($_POST['referring_url']) : '';}	
                return $valid['referring_url'];
			//	isset($_POST['referring_url']) ? $_POST['referring_url'] : '';
    ...
    public function setStyles(array $styles)
    {
        foreach ($styles as $style) {
            if (isset($style['type']) && isset($style['css'])) {
                $this->_styles[$style['type']] = vowelsforminc::parseCss($style['css']);
            }
        }
    }
    ...
    public static function parseCss($css)
    {
        ...
        if (strlen($css) > 0) {
            try {
                $parser = new TCRCSSParser('vowels { ' . $css . ' }');
                $doc = $parser->parse();
                $parsed = array();
    ...
    {
        foreach ($this->_elements as $element) {
            if ($element instanceof vowelsforminc_Element_File && $element->getEnableSwfUpload()) {
                return true;
            }
        }
    ...
    public function calculateElementStatus()
    {
        ...
        foreach ($this->_elements as $element) {
            ...
            if ($element instanceof vowelsforminc_Element_Groupend) {
            ...
            if (!$isConditionallyHidden && $element->getLogic() && $ruleCount = count($rules = $element->getLogicRules())) {
                ...
                for ($i = 0; $i < $ruleCount; $i++) {
                    ...
                    $otherElementHasValue = $otherElement instanceof vowelsforminc_Element && $otherElement->hasValue($rule['value']);
