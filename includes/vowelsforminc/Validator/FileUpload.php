...
defined('VOWELSFORMDRAGDROP_UPLOAD_ERR_TYPE') || define('VOWELSFORMDRAGDROP_UPLOAD_ERR_TYPE', 128);
defined('VOWELSFORMDRAGDROP_UPLOAD_ERR_FILE_SIZE') || define('VOWELSFORMDRAGDROP_UPLOAD_ERR_FILE_SIZE', 129);
defined('VOWELSFORMDRAGDROP_UPLOAD_ERR_NOT_UPLOADED') || define('VOWELSFORMDRAGDROP_UPLOAD_ERR_NOT_UPLOADED', 130);

/**
 * vowelsforminc_Validator_FileUpload
 *
 ...
 */
class vowelsforminc_Validator_FileUpload extends vowelsforminc_Validator_Abstract
{
    /**
    ...
    public function __construct($options)
    {
        $this->_messageTemplates = array(
            'not_uploaded_with_filename' => __('File \'%s\' is not an uploaded file', 'vowels-contact-form-with-drag-and-drop'),
            'not_uploaded' => __('File is not an uploaded file', 'vowels-contact-form-with-drag-and-drop'),
            'too_big_with_filename' => __('\'%s\' exceeds the maximum allowed file size', 'vowels-contact-form-with-drag-and-drop'),
            'too_big' => __('File exceeds the maximum allowed file size', 'vowels-contact-form-with-drag-and-drop'),
            'not_allowed_type_with_filename' => __('File type of \'%s\' is not allowed', 'vowels-contact-form-with-drag-and-drop'),
            'not_allowed_type' => __('File type is not allowed', 'vowels-contact-form-with-drag-and-drop'),
            'field_required' => __('This field is required', 'vowels-contact-form-with-drag-and-drop'),
            'only_partial_with_filename' => __('\'%s\' was only partially uploaded', 'vowels-contact-form-with-drag-and-drop'),
            'only_partial' => __('File was only partially uploaded', 'vowels-contact-form-with-drag-and-drop'),
            'no_file' => __('No file was uploaded', 'vowels-contact-form-with-drag-and-drop'),
            'missing_temp_folder' => __('Missing a temporary folder', 'vowels-contact-form-with-drag-and-drop'),
            'failed_to_write' => __('Failed to write file to disk', 'vowels-contact-form-with-drag-and-drop'),
            'stopped_by_extension' => __('File upload stopped by extension', 'vowels-contact-form-with-drag-and-drop'),
            'unknown_error' => __('Unknown upload error','vowels-contact-form-with-drag-and-drop')
        );
        ...
        if (is_array($file['error'])) {
            ...
            foreach ($file['error'] as $key => $error) {
                if ($error === UPLOAD_ERR_OK) {
                    ...
                    if (!vowels_form_builder_is_uploaded_file($file['tmp_name'][$key])) {
                        // The file is not an uploaded file - possibly an attack
                        $this->addMessage($this->_getFileUploadError(VOWELSFORMDRAGDROP_UPLOAD_ERR_NOT_UPLOADED, $file['name'][$key]));
                        return false;
                    }

                    if ($this->_maximumFileSize > 0 && $file['size'][$key] > $this->_maximumFileSize) {
                        // The file is larger than the size allowed by the settings
                        $this->addMessage($this->_getFileUploadError(VOWELSFORMDRAGDROP_UPLOAD_ERR_FILE_SIZE, $file['name'][$key]));
                        return false;
                    }
                    ...
                    if (count($this->_allowedExtensions) && !in_array($extension, $this->_allowedExtensions)) {
                        ...
                        $this->addMessage($this->_getFileUploadError(VOWELSFORMDRAGDROP_UPLOAD_ERR_TYPE, $file['name'][$key]));
                        return false;
                    }
                    $uploaded++;
            ...
            if ($file['error'] === UPLOAD_ERR_OK) {
                // The file uploaded OK
                if (!vowels_form_builder_is_uploaded_file($file['tmp_name'])) {
                    // The file is not an uploaded file - possibly an attack
                    $this->addMessage($this->_getFileUploadError(VOWELSFORMDRAGDROP_UPLOAD_ERR_NOT_UPLOADED));
                    return false;
                }
                ...
                if ($this->_maximumFileSize > 0 && $file['size'] > $this->_maximumFileSize) {
                    ...
                    $this->addMessage($this->_getFileUploadError(VOWELSFORMDRAGDROP_UPLOAD_ERR_FILE_SIZE));
                    return false;
                }
                ...
                if (count($this->_allowedExtensions) > 0 && !in_array($extension, $this->_allowedExtensions)) {
                    // The file extension is not allowed
                    $this->addMessage($this->_getFileUploadError(VOWELSFORMDRAGDROP_UPLOAD_ERR_TYPE));
                    return false;
                }
    ...
    protected function _getFileUploadError($errorCode, $filename = '')
    {
        switch ($errorCode) {
            ...
            case VOWELSFORMDRAGDROP_UPLOAD_ERR_FILE_SIZE:
                if (strlen($filename)) {
            ...
            case VOWELSFORMDRAGDROP_UPLOAD_ERR_TYPE:
                if (strlen($filename)) {
                ...
                return $message;
            case VOWELSFORMDRAGDROP_UPLOAD_ERR_NOT_UPLOADED:
                if (strlen($filename)) {
