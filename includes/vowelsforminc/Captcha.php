...
class vowelsforminc_Captcha
{
            ...
            if (array_key_exists('font', $options)) {
                if (file_exists($options['font'])) {
                    if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                        ...
                        $vowelsFontsTmpDir = $tmpDir . '/vowels/fonts';
                        $cachedFontPath = $vowelsFontsTmpDir . '/' . $filename;

                        if (file_exists($cachedFontPath)) {
                            $this->_font = $cachedFontPath;
                        } else {
                            if (!is_dir($vowelsFontsTmpDir)) {
                                wp_mkdir_p($vowelsFontsTmpDir);
                            }

                            if (is_writable($vowelsFontsTmpDir)) {
    ...
    public function display()
    {
        ...
        $_SESSION['vowels-captcha-' . $this->_uniqId] = $this->_code;
    }
