...
class vowelsforminc_Element
{
    ...
    public function getLabelHtml($tooltipType, $tooltipEvent, $labelCss, $for = true, $id = '')
    {
        ...
        $output .= '<label' . ($id ? ' id="' . esc_attr($id) . '"' : '') .
                    ...
                    ' class="' . esc_attr($this->getName()) . '-outer-label vowelsmtop" ' . $this->getCss('label', $labelCss) . ' style="margin-bottom: 0em;margin-top: 0em;">';
        $output .= $label;

        if ($this->getRequired() && strlen($requiredText = $this->_form->getRequiredText()))  {
            $output .= '<span class="vowels-required">' . esc_html($requiredText) . '</span>';
        }

        if ($this->_form->getUseTooltips() && strlen($this->getTooltip()) && $tooltipType == 'icon') {
            $output .= '<span class="vowels-tooltip-icon vowels-tooltip-icon-' . $tooltipEvent . '"><span class="vowels-tooltip-icon-content">' . $this->getTooltip() . '</span></span>';
        }
    ...
    public function setRequiredMessage($requiredMessage)
    {
        if ($requiredMessage) {
            ...
            if ($requiredValidator instanceof vowelsforminc_Validator_Required) {
     ...
     * Add a filter
     *
     * @param string|vowelsforminc_Filter_Interface $filter The name or instance of the filter
     */
    public function addFilter($filter, $options = null)
    {
        ...
        if ($filter instanceof vowelsforminc_Filter_Interface) {
            $name = get_class($filter);
            $this->_filters[$name] = $filter;
        } else {
            throw new Exception('Filter passed to addFilter must be a string or instance of vowelsforminc_Filter_Interface');
        }
    }
    ...
    public function addFilters(array $filters)
    {
        foreach ($filters as $filter) {
            ...
            } else if ($filter instanceof vowelsforminc_Filter_Interface) {
                $this->addFilter($filter);
    ...
    public function hasFilter($filter)
    {
        ...
        if (is_string($filter)) {
            if (strpos($filter, 'vowelsforminc_Filter_') === false) {
                $filter = 'vowelsforminc_Filter_' . ucfirst($filter);
            }
     ...
     * @return vowelsforminc_Filter_Interface|null The filter or null if the element does not have the filter
     */
     ...
     * Add a validator
     *
     * @param string|vowelsforminc_Validator_Interface $validator The validator to add
     */
    public function addValidator($validator, $options = null)
    {
        ...
        if ($validator instanceof vowelsforminc_Validator_Interface) {
            $name = get_class($validator);
            $this->_validators[$name] = $validator;
        } else {
            throw new Exception('Validator passed to addValidator must be a string or instance of vowelsforminc_Validator_Interface');
        }
    }
    ...
    public function addValidators(array $validators)
    {
        foreach ($validators as $validator) {
            ...
            } else if ($validator instanceof vowelsforminc_Validator_Interface) {
                $this->addValidator($validator);
     ...
     *
     * @param string|vowelsforminc_Validator_Interface $name The name or instance of the validator
     * @return boolean
     */
    public function hasValidator($validator)
    {
        ...
        if (is_string($validator)) {
            if (strpos($validator, 'vowelsforminc_Validator_') === false) {
                $validator = 'vowelsforminc_Validator_' . ucfirst($validator);
            }
     ...
     * @return vowelsforminc_Validator_Interface|null The validator or null if the element does not have the validator
     */
    ...
    public function isValid()
    {
        ...
        if (!$skipValidation) {
            ...
            $valid = apply_filters('vowels_form_builder_element_valid', $valid, $value, $this);
            $valid = apply_filters('vowels_form_builder_element_valid_' . $this->getName(), $valid, $value, $this);
        }
     ...
     */
    public function setForm(vowelsforminc $form)
    {
        $this->_form = $form;
    }
    ...
    public function fromConfig(array $config)
    {
        ...
        if (array_key_exists('filters', $config) && is_array($config['filters'])) {
            foreach ($config['filters'] as $fConfig) {
                if (isset($fConfig['type'])) {
                    $fClass = 'vowelsforminc_Filter_' . ucfirst($fConfig['type']);
        ...
        if (array_key_exists('validators', $config) && is_array($config['validators'])) {
            foreach ($config['validators'] as $vConfig) {
                if (isset($vConfig['type'])) {
                    $vClass = 'vowelsforminc_Validator_' . ucfirst($vConfig['type']);
        ...
        if (array_key_exists('duplicate_found_message', $config) && strlen($config['duplicate_found_message'])) {
            ...
            if ($duplicateValidator instanceof vowelsforminc_Validator_Duplicate) {
    ...
    public function setDynamicDefaultValue($key)
    {
        ...
        $value = apply_filters('vowels_form_builder_element_value', $value, $key);
        $value = apply_filters('vowels_form_builder_element_value_' . $key, $value, $key);
     ...
     * @return null|vowelsforminc_Filter_Interface The filter
     */
    protected function _loadFilter($filter, $options = null)
    {
        ...
        if (!empty($filter)) {
            $className = 'vowelsforminc_Filter_' . ucfirst($filter);
     ...
     * @return null|vowelsforminc_Validator_Interface The validator
     */
    protected function _loadValidator($validator, $options = null)
    {
        ...
        if (!empty($validator)) {
            $className = 'vowelsforminc_Validator_' . ucfirst($validator);
            if (class_exists($className)) {
                $instance = new $className($options);
            }
        }
