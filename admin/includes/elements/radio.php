...
if (!isset($element['options'])) {
    $element['options'] = array(
        array('value' => __('Option 1', 'vowels-contact-form-with-drag-and-drop'), 'label' => __('Option 1','vowels-contact-form-with-drag-and-drop')),
        array('value' => __('Option 2', 'vowels-contact-form-with-drag-and-drop'), 'label' => __('Option 2','vowels-contact-form-with-drag-and-drop')),
        array('value' => __('Option 3', 'vowels-contact-form-with-drag-and-drop'), 'label' => __('Option 3','vowels-contact-form-with-drag-and-drop'))
    );
}
$count = count($element['options']);
$helpUrl = vowels_form_builder_help_link('element-radio');
?>
	...
	<div class="ifb-top-element-wrap qfb-cf">
        ...
        <div class="ifb-element-preview ifb-element-preview-radio">
            ...
            <div class="ifb-preview-input">
                <ul id="ifb_element_<?php echo $id; ?>" class="ifb-radio-option-list <?php echo ($element['options_layout'] == 'block') ? 'ifb-options-block' : 'ifb-options-inline'; ?> qfb-cf">
                    <?php if ($count) : ?>
                        <?php $i = 0; foreach ($element['options'] as $option) : ?>
                            <li><input type="radio" name="ifb_element_<?php echo $id; ?>" disabled="disabled" class="preview_form" value="<?php echo esc_attr($option['value']); ?>" <?php echo in_array($option['value'], $element['default_value'], true) ? 'checked="checked"' : ''; ?> /><label><?php echo $option['label']; ?></label></li>
                            <?php if ($i == 4) break; ?>
                        <?php $i++; endforeach; ?>
                    <?php endif; ?>
                </ul>
                <p class="ifb-options-overflow <?php if ($count < 5) echo 'ifb-hidden'; ?>" id="ifb_options_overflow_<?php echo $id; ?>"><?php esc_html_e('Only the first 5 options are shown here.','vowels-contact-form-with-drag-and-drop'); ?></p>
                        ...
                        <tr valign="top">
                            <th scope="row">
                                <div class="ifb-tooltip"><div class="ifb-tooltip-content">
                                    ...
                                    choices. Most of your settings will also be converted.','vowels-contact-form-with-drag-and-drop'); ?>
                                </div></div>
                                <label for="convert_element_<?php echo $id; ?>"><?php esc_html_e('Convert element','vowels-contact-form-with-drag-and-drop'); ?></label>
                            </th>
                            <td>
                                <select name="convert_element_<?php echo $id; ?>" id="convert_element_<?php echo $id; ?>" onchange="vowelsforminc.convertElement(vowelsforminc.getElementById(<?php echo $id; ?>), this); ">
                                    <option value=""><?php esc_html_e('Please select...','vowels-contact-form-with-drag-and-drop'); ?></option>
                                    <option value="select"><?php esc_html_e('Dropdown Menu','vowels-contact-form-with-drag-and-drop'); ?></option>
                                    <option value="checkbox"><?php esc_html_e('Checkboxes','vowels-contact-form-with-drag-and-drop'); ?></option>
                                </select>
                            </td>
                        </tr>
                        <?php include '_save.php'; ?>
