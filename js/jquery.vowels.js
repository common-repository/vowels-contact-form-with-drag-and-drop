/*
 * vowelsforminc WordPress jQuery plugin
 */
;(function($) {
	$.vowelsforminc = function ($form, options) {
		var _this = this,
			settings = {},
			extraData = {
				vowels_form_builder_ajax: 1
			},
			submitted = false,
			uploadQueue = [],
			uploadElements = [],
			$successMessage = $('.vowels-success-message', $form),
			$loadingSpinner = $('.vowels-loading-wrap', $form),
			clCache = {},
		...
		_this.addUploader = function (element) {
			...
			function uploadAdd(e, data) {
				...
				if (element.fileUploadLimit && element.queued === element.fileUploadLimit) {
					uploadAddError(vowelsL10n.upload_too_many);
					return;
				}

				// Validate file type (client-side)
				if (element.allowedExtensions && ! isAllowedFile(file, element.allowedExtensions)) {
					uploadAddError(file.name + ' - ' + vowelsL10n.upload_file_type_not_allowed);
					return;
				}

				// Validate file size (client-side)
				if (element.fileSizeLimit && file.size > element.fileSizeLimit) {
					uploadAddError(file.name + ' - ' + vowelsL10n.upload_file_too_big);
					return;
				}
				...
				data.formData = {
					vowels_form_builder_ajax_uploading: 1,
					vowels_form_builder_id: settings.id,
					vowels_form_builder_uid: settings.uniqueId,
					vowels_form_builder_element_id: element.id,
					vowels_form_builder_upload_id: uploadId
				};
				...
				var $file = data.context = $('<div class="vowels-upload-queue-file">').append($('<div class="vowels-upload-queue-filename">').text(filename));

				$('<div class="vowels-upload-queue-remove">X</div>').data('upload-id', uploadId).click(function () {
					var i = uploadQueue.length,
						uploadId = $(this).data('upload-id');

					while (i--) {
						if (uploadQueue[i].formData.vowels_form_builder_upload_id == uploadId) {
			...
			function uploadAddError(message) {
				var $queueErrorsList = $('<div class="vowels-queue-errors-list vowels-clearfix"></div>');
				$queueErrorsList.append('<div class="vowels-queue-error">' + message + '</div>');
				$queueErrors.append($queueErrorsList).show();
			}

			function uploadStart() {
				$('.vowels2-upload-progress-wrap', $form).show();
			}

			function uploadProgress(e, data) {
				var progress = Math.min(100, ((data.loaded / data.total) * 100)); // Limit to 100% maximum

				$('.vowels-upload-progress-bar', $form).css('width', progress + '%');
				$('.vowels-upload-filename', $form).text(data.files[0].name);
			}

			$input.fileupload({
				url: vowelsL10n.siteUrl,
				dataType: 'json',
			...
			upload.submit().done(function (response) {
				if (typeof response == 'object' && response !== null && typeof response.type == 'string') {
					if (response.type == 'success') {
						...
						upload.context.find('.vowels-upload-queue-remove').removeClass('vowels-upload-queue-remove').addClass('vowels-upload-queue-success').unbind('click');
						...
						if (uploadQueue.length > 0) {
							...
							$('.vowels2-upload-progress-wrap', $form).hide();
							_this.submit();
						}
					} else {
					...
					uploadQueue.unshift(upload);
					uploadError(vowelsL10n.invalid_response);
				}
			})
			.fail(function () {
				uploadQueue.unshift(upload);
				uploadError(vowelsL10n.ajax_error);
			});
		};
		...
		function uploadError(message) {
			...
			$('.vowels-upload-error', $form).text(message).show();
		}
		...
		_this.preSubmit = function() {
			...
			if (settings.preview === true) {
				alert(vowelsL10n.preview_no_submit);
				return false;
			}
			...
			$('.vowels-upload-error', $form).hide().text('');
		...
		if (settings.useAjax) {
			$form.bind('submit', function (event) {
				...
				var $recaptcha = $form.find('.vowels-recaptcha');

				if ($recaptcha.length) {
					var recaptchaConfig = $recaptcha.data('config');

					if (recaptchaConfig.size == 'invisible') {
						grecaptcha.execute($recaptcha.data('vowels-recaptcha-id'));
						return;
					}
				}
		...
		var prepareForm = function () {
			...
			$('.vowels-errors-wrap', $form).hide();
			$('.vowels-errors-list, .vowels-error', $form).remove();
			$('.vowels-queue-errors', $form).hide().empty();
			$successMessage.hide();
			$('.vowels-element-error', $form).removeClass('vowels-element-error');
			...
			$form.ajaxSubmit({
				...
				success: function(response) {
					...
					if (!!window.grecaptcha) {
						$('.vowels-recaptcha', $form).each(function () {
							try {
					...
					if (response === null || response === undefined) {
						...
						alert(vowelsL10n.error_submitting_form);
						...
						if (response.type == 'success') {
							...
							$.each(response.data, function(index, info) {
								...
								if ($.isArray(info.errors) && info.errors.length > 0) {
									...
									$errorsWrap = $elementWrap.find('.vowels-errors-wrap');

									// If the returned element exists
									if ($elementWrap.length && $errorsWrap.length) {
										// Create a blank error list
										var $errorList = $('<div class="vowels-errors-list vowels-clearfix"></div>');

										// Go through each error for this field
										$.each(info.errors, function(i, e) {
											// Append the error to the list as a list item
											$errorList.append('<div class="vowels-error">' + e + '</div>');
										...
										$elementWrap.addClass('vowels-element-error');
									}
								}
							});

							// Fade all errors in
							$('.vowels-errors-wrap', $form).fadeIn(1000).show();
							...
							$form.trigger('vowels:error', [$errors, response]);
		...
		_this.resetForm = function (resetFormValues) {
			...
			// Reset the captcha
			$('.vowels-captcha-image', $form).trigger('click');
			...
			$('.vowels-add-another-file-wrap', $form).remove();
		...
		_this.addDatepicker = function (uniqueId, options) {
			if ($.isFunction($.fn.datepicker)) {
				...
				$datePicker = $('.vowels-datepicker', '#' + uniqueId).datepicker($.extend({}, {
				...
				$('.vowels-datepicker-icon', '#' + uniqueId).click(function () {
		...
		_this.applyElementLogic = function (elementId, loading) {
			if (vowelsforminc.logic[settings.id] && vowelsforminc.logic[settings.id].logic) {
				var logic = vowelsforminc.logic[settings.id].logic[elementId];

				if (logic && logic.rules && logic.rules.length) {
					var matchedValues = 0, elementUniqueIdPrefix = 'vowels_form_builder_' + settings.id + '_', action;
					...
					var $element = $('.vowels_form_builder_'+settings.id+'_'+elementId+'-element-wrap', $form);

					if ($element.length === 0) {
						$element = $('.vowels_form_builder_'+settings.id+'_'+elementId+'-group-wrap', $form);
					}

					if (!loading && vowelsforminc.logic[settings.id].animate) {
						if (action == 'show') {
							$element.slideDown(20, function () {
								centerFancybox();
							});
						} else {
							$element.slideUp(20, function () {
								centerFancybox();
							});
						}
					} else {
		...
		_this.applyDependentLogic = function (elementIds) {
			if (vowelsforminc.logic[settings.id] && vowelsforminc.logic[settings.id].dependents) {
				var i = 0,
				...
				for ( ; i < length; i++) {
					var dependentElementIds = vowelsforminc.logic[settings.id].dependents[elementIds[i]],
						elementUniqueId = 'vowels_form_builder_' + settings.id + '_' + elementIds[i],
		 ...
		 *
		 * @param   {string}   elementUniqueId  The unique ID of the element e.g. vowels_form_builder_1_1
		...
		_this.clearDefaultValue = function (reset) {
			$(this).data('vowels-default-value', $(this).val()).val('').unbind('focus');
		...
		_this.resetDefaultValue = function () {
			if ($(this).val() === '') {
				$(this).val($(this).data('vowels-default-value')).unbind('blur').bind('focus', function () {
	...
	// jQuery plugin
	$.fn.vowelsforminc = function(options) {
		return this.each(function () {
			var $this = $(this);
			if (!$this.data('vowelsform')) {
				var form = new $.vowelsforminc($this, options);
				vowelsforminc.instance = form;
				$this.data('vowelsform', form);
			}
		});
	};

	// Callback to render reCAPTCHA
	window.vowelsformincRecaptchaLoaded = function () {
		if (!window.grecaptcha) {
			return;
		}

		$('.vowels-recaptcha').each(function () {
			var $this = $(this),
				config = $this.data('config');

			if (config.size == 'invisible') {
				config.callback = function () {
					$this.closest('.vowels').data('vowelsform').preSubmit();
				};
			}

			$this.data('vowels-recaptcha-id', grecaptcha.render($this.data('unique-id'), config));
		});
	};

	// Preload the images in the base theme
	$(window).on('load', function () {
		window.vowelsforminc.preload([
			'file-upload-tick.png',
			'captcha-refresh-icon.png',
			'default-loading.gif',
			'error.png',
			'success.png'
		], vowelsL10n.plugin_url + '/images/');
	});
